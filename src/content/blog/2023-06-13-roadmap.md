...
- :white_check_mark: ~~searching within website (fully local, offline-capable client search using Pagefind client-side search with dynamic search-as-you-type)~~
...
- :white_check_mark: ~~Load Markmap and Pagefind scripts locally rather than external CDNs~~
- :white_check_mark: ~~Full Lighthouse audit optimization (100% scores across all categories with strict CSP)~~
- :white_check_mark: ~~Migrate client-side search from Lunr to Pagefind~~
- :white_check_mark: ~~Remove Alpine.js and replace all toggles with native Vanilla JS~~
- :white_check_mark: ~~Add comprehensive unit (Vitest) and End-to-End (Playwright) testing suite~~
