...
1. Clone your own version of the starter template or fork the repository. Run `pnpm install` then `pnpm run build` or `pnpm run dev`.
...
15. If you make changes to the files, use `pnpm run lint` to format the workspace using Prettier and auto-fix linter issues using ESLint.
16. `pnpm run check` will check the source code compile correctness and type safety.
17. Run the comprehensive test suite using `pnpm test` (which executes unit tests via Vitest and End-to-End browser tests via Playwright) to verify code correctness.
